...
from .forms import AddVehicleForm, AddCustomerForm, AddRentalForm
...
def rental(request):
    return render(request, 'rental.html', {'rental': Rental.objects.all()})


def rental_id(request):
...
def vehicle(request):
    return render(request, 'vehicle.html', {'vehicle': VehicleType.objects.all()})
    return render(request, 'vehicle.html', {'vehicle': VehicleSize.objects.all()})
